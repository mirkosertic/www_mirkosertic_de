...
For security reasons, where is no way to introspect all life local variables in WebAssembly. To get around this issue,
...
Arrays are a different beast. They are basically instances, but their length cannot determined at compile time in every
