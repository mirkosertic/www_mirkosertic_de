...
like "Test coverage" to get insight about the completeness of our tests. A very common asumption taken from a test coverage of 80%
is that 80% of our code is tested and working correctly.
...
It just proves that 80 percent of the example above were executed by our unit tests. It doesn't say that the 80% are working
...
a test coverage of 80% only proves for sure that 20% of our code is definitely not tested at all.

== What to do?

Well, we need another metric to measure the effectiveness of our tests, so we know that the 80% test coverage is indeed able to
...
Now, let us see what happens if the apply some mutation testing. We get the following report:
...
Also, it is not good to insist on a 100% mutation coverage, as this misses the same point as insisting on a 100%
